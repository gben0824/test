...
private:
    ...
    ros::Subscriber sub_lidar, sub_livox_left, sub_livox_right,sub_lidar_front, sub_odom;
    ...
    pcl::PointCloud <PointType> cloud_filtered1; //离群点滤波用

    pcl::PointCloud<PointType>::Ptr pointcloud_adder; // 点云累加
    pcl::PointCloud<PointType>::Ptr pointcloud_adder_front; // 点云累加

    pcl::PointCloud <PointType> laserCloud_ds;
    pcl::PointCloud <PointType> outlinerremove_ds;
    ...
    message_filters::Subscriber <sensor_msgs::PointCloud2> sub_lidar_front_; //中间雷达订阅
    message_filters::Subscriber <nav_msgs::Odometry> sub_odom_; //rtk的odom订阅
    typedef message_filters::sync_policies::ApproximateTime <sensor_msgs::PointCloud2, sensor_msgs::PointCloud2, sensor_msgs::PointCloud2, sensor_msgs::PointCloud2, nav_msgs::Odometry> MySyncPolicy;
    ...
    pcl::ModelCoefficients::Ptr coefficients_front; // ransac用
    pcl::PointIndices::Ptr inliers_front;
    pcl::PointCloud <PointType> laserCloudGround_front;
    ...
    pcl::PointCloud <pcl::PointXYZ> boundary_point_front_local; // 算法计算出的路面边缘点(3帧的)
    ...
    pcl::PointCloud <pcl::PointXYZ> boundary_front_inMap; // 在odom或map下的坐标(3帧的)

    pcl::PointCloud <pcl::PointXYZ> left_Pts_added; // 各帧的左边沿点叠加在一起
    pcl::PointCloud <pcl::PointXYZ> right_Pts_added; // 各帧的右边沿点叠加在一起
    pcl::PointCloud <pcl::PointXYZ> front_Pts_added; // 各帧的右边沿点叠加在一起
...
public:
            ...
            first_frame(true)
    {
        ...
        coefficients_front.reset(new pcl::ModelCoefficients);
        inliers.reset(new pcl::PointIndices);
        inliers_front.reset(new pcl::PointIndices);
        tree.reset(new pcl::search::KdTree <PointType>);
        pointcloud_adder.reset(new pcl::PointCloud <PointType>);  // 初始化
        pointcloud_adder_front.reset(new pcl::PointCloud <PointType>);  // 初始化
        ...
        sub_lidar_front_.subscribe(nh, "/livox/lidar_front", 5);
        sub_odom_.subscribe(nh, "/gps/odom", 20); // topic名称改了
        pub_final = nh.advertise<sensor_msgs::PointCloud2>("/road_boundary", 10);
        pub_front = nh.advertise<sensor_msgs::PointCloud2>("/road_front", 10);
        ...
        //回调
        sync_.reset(new Sync(MySyncPolicy(10), sub_lidar_, sub_lidar_left_, sub_lidar_right_,sub_lidar_front_, sub_odom_));
        sync_->registerCallback(boost::bind(&BoundaryDetector::callback, this, _1, _2, _3, _4, _5));
    }
    ...
    void callback(const sensor_msgs::PointCloud2ConstPtr &laserCloudMsg,
                  ...
                  const sensor_msgs::PointCloud2ConstPtr &laserCloudMsgFront,
                  const nav_msgs::Odometry::ConstPtr &odomMsg);

    void planeFitting(pcl::PointCloud <PointType> pc_filtered);

    void planeFitting1(pcl::PointCloud <PointType> pc_filtered1);  //点云去除离群点后的地面分割
...
//  将3个雷达点云的点云合为1个
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr laserCloudLivoxFront(new pcl::PointCloud <PointType>);
    ...
    pcl::fromROSMsg(*laserCloudMsgFront, *laserCloudLivoxFront);
    indices.clear();
    pcl::removeNaNFromPointCloud(*laserCloudLivoxFront, *laserCloudLivoxFront, indices);  // 去除NaN点
    laserCloudIn = laserCloudIn + *laserCloudLivoxFront; // 4个雷达点云叠加

    laserCloudMiddle->clear();
    laserCloudLivoxLeft->clear();
    laserCloudLivoxRight->clear();
    laserCloudLivoxFront->clear();
    ...
    sor.setInputCloud(laserCloudIn.makeShared());
    sor.setMeanK(100);
    sor.setStddevMulThresh(0.5);
    sor.filter(cloud_filtered1);

    /*pcl::PassThrough <PointType> pass_front;
    pass_front.setInputCloud(cloud_filtered1.makeShared());
    pass_front.setFilterFieldName("x");
    // X_min小可以充分发挥补盲雷达的作用。
    pass_front.setFilterLimits(0, 50);
    pass_front.filter(cloud_filtered1);
*/
/*
    pcl::PassThrough <PointType> pass_front1;
    pass_front1.setInputCloud(cloud_filtered1.makeShared());
    pass_front1.setFilterFieldName("y");
    // X_min小可以充分发挥补盲雷达的作用。
    pass_front1.setFilterLimits(-30, 30);
    pass_front1.filter(cloud_filtered1);
*/
...
//        std::cout << "roll pitch is " << roll*57.0 << ' ' << pitch*57.0 << std::endl;
    ...
    *pointcloud_adder_front = *pointcloud_adder_front + cloud_filtered1;
    frame_cnt++;
...
//        pub_debug.publish(pc_debug);   // topic name
        ...
        // RANSAC地面分割，得到地面点云和非地面点云
        planeFitting1(*pointcloud_adder_front);
        // 下采样
        ...
        filter1.setInputCloud(laserCloudGround_front.makeShared());
        // 设置体素栅格的大小
        filter1.setLeafSize(1, 1, 1);
        filter1.filter(outlinerremove_ds);
        // 发布所有的下采样后点云
...
//        std::cout<< "transform_1:\n" << transform_1 <<std::endl << std::endl;
        ...
        pcl::transformPointCloud(boundary_point_front_local, boundary_front_inMap, transform_1);
        boundary_front_inMap.header.frame_id = "map";
        boundary_front_inMap.width = boundary_front_inMap.points.size();
        boundary_front_inMap.height = 1;
        boundary_front_inMap.is_dense = true;
        // 合成
        front_Pts_added += boundary_front_inMap;
        front_Pts_added.header.frame_id = "map";
        front_Pts_added.width = front_Pts_added.points.size();
        front_Pts_added.height = 1;
        front_Pts_added.is_dense = true;

        boundary_point_left_local.clear();  // 将每3帧融合后的边界点云清空（这是base_link下的）
        boundary_point_right_local.clear();
        boundary_point_front_local.clear();

        boundary_left_inMap.clear();
        boundary_right_inMap.clear();
        boundary_front_inMap.clear();
...
//全部点云去掉离群点后的地面分割
void BoundaryDetector::planeFitting1(pcl::PointCloud <PointType> pc_filtered1) {
    // 创建分割对象
    pcl::SACSegmentation <PointType> opt;
    // 可选设置
    opt.setOptimizeCoefficients(true);
    opt.setMaxIterations(200);  // 循环次数
    // 必须设置
    opt.setModelType(pcl::SACMODEL_PERPENDICULAR_PLANE);
    opt.setMethodType(pcl::SAC_RANSAC);
    opt.setDistanceThreshold(ransac_th); // 从内点到拟合平面的距离 0.2
    opt.setAxis(Eigen::Vector3f(0.0, 0.0, 1.0));
    opt.setEpsAngle(12.0 / 180.0 * PI);

    opt.setInputCloud(pc_filtered1.makeShared());  //输入为点云指针
    opt.segment(*inliers_front, *coefficients_front);
    pcl::ExtractIndices <PointType> extract;
    if (inliers->indices.size() < 300)
    {
        ROS_INFO("Too small inliers in RANSAC model, skip!"); // 排除近处的密集点
    ...
    {
        extract.setInputCloud(pc_filtered1.makeShared());
        extract.setIndices(inliers_front);
        extract.filter(laserCloudGround_front);  // 地面点云
//        std::cout << "Number of inlier points:" << laserCloudGround.points.size() << std::endl;
//        double ratio = double(laserCloudGround.points.size())/double(pc_filtered.points.size());
//        std::cout << "ratio is :" << ratio << std::endl;
//        extract.setNegative (true);
...
void BoundaryDetector::PickBoundary(pcl::PointCloud<pcl::PointXYZ>::ConstPtr pc_cluster) {
    /*vector<vector<double> > r_vec(120,vector<double>(10000,0));
    double angle1 , r1;
    int flag;
    vector<double> r_list;
    vector<double> r_part(10000,0);
    vector<double> ang_list;*/
   // r_list.resize(outlinerremove_ds.size(),0);
    //ang_list.resize(outlinerremove_ds.size(),-1000);
...
//        if (theta_left>0.0 && theta_left<38.0 && (abs(left_point.y - last_left_point_oneSlice.y) < 2.0))
        ...
        /* ----扇形挑选边界点-----*/
        /*for (int num = 0; num < outlinerremove_ds.size() - 1; ++num) {
            //r_vec[num].resize(outlinerremove_ds.size(),0);
            angle1 = atan2(outlinerremove_ds.points[num].y,outlinerremove_ds.points[num].x);
            r1  = sqrt(outlinerremove_ds.points[num].x * outlinerremove_ds.points[num].x
                       + outlinerremove_ds.points[num].y * outlinerremove_ds.points[num].y);
            flag = int(angle1) / 1.5;
            r_vec[flag][int(num / 10)] = r1;
        }
        for(int i = 0; i < r_vec.size() ; ++i){
            vector<int> r_index(10000,0) ;
            for (int l = 0; l < r_vec[i].size()-1; ++l) {
                r_part.push_back(r_vec[i][l]);
            }
            for (int j = 0; j !=r_index.size() ; ++j) {
                r_index[j] = j;
            }
            sort(r_index.begin(),r_index.end(),[&](const int& a,const int& b){
                return (r_part[a] > r_part[b]);});  //输出r_vec[i]行中由大到小排序后的索引
            boundary_point_front_local.points.push_back(outlinerremove_ds.points[r_index[0] * 10]);
...
/*    boundary_point_front_local.width = boundary_point_front_local.points.size();
    boundary_point_front_local.height = 1;
    boundary_point_front_local.is_dense = true;
    boundary_point_front_local.header.frame_id = "base_link";*/
...
void BoundaryDetector::PickBoundary_front() {
    ...
    for (int num = 0; num < outlinerremove_ds.size() - 1; ++num) {
        //  r_vec[num].resize(outlinerremove_ds.size(),0);
        angle1 = atan2(outlinerremove_ds.points[num].x,outlinerremove_ds.points[num].y) * 57.3;  //转成角度
        r1  = sqrt(outlinerremove_ds.points[num].x * outlinerremove_ds.points[num].x
                + outlinerremove_ds.points[num].y * outlinerremove_ds.points[num].y);
        flag = int(angle1) ;
        if(first)
        ...
        //r_vec[flag][int(num)] = r1;
    }
    for (int i = 0; i < 180; ++i) {
        boundary_point_front_local.points.push_back(outlinerremove_ds.points[ r_num[i][0] ]);
    }
        /*for(int i = 0; i < r_vec.size() ; ++i){
            vector<int> r_index(10000,0) ;
            for (int l = 0; l < r_vec[i].size()-1; ++l) {
            ...
            sort(r_index.begin(),r_index.end(),[&](const int& a,const int& b){
                return (r_part[a] > r_part[b]);});  //输出r_vec[i]行中由大到小排序后的索引
            boundary_point_front_local.points.push_back(outlinerremove_ds.points[r_index[0] ]);

            r_part.clear();
            r_index.clear();
        }*/

        /*for (int i = 0; i < outlinerremove_ds.size() - 1; ++i) {
            angle1 = atan2(outlinerremove_ds.points[i].y,outlinerremove_ds.points[i].x);
            ang_list.push_back(angle1);
            if(ang_list[i] - ang_list[0] < 2.0 && ang_list[i] - ang_list[0] >= 0)
            {
                r1  = sqrt(outlinerremove_ds.points[i].x * outlinerremove_ds.points[i].x
                           + outlinerremove_ds.points[i].y * outlinerremove_ds.points[i].y);
                r_list.push_back(r1);
            } else if(r_list.size() > 100)
            {
                vector<int> r_index(r_list.size(),0);
                for (int j = 0; j !=r_index.size() ; ++j) {
                    r_index[i] = i;
                }
                sort(r_index.begin(),r_index.end(),[&](const int& a,const int& b){
                    return (r_list[a] > r_list[b]);});  //输出r_list由大到小排序后的索引
...
//    // 发布累加的左边沿点
    ...
    pcl::PointCloud<PointType>::Ptr cloud_select_front(new pcl::PointCloud <PointType>);
...
//    Pt_temp.y = centerPt(1);
    ...
    pcl::PointCloud<PointType> cloud_temp, cloud_temp_left, cloud_temp_right, cloud_temp_front;
...
/*
    // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 开始处理扇形边界点
    // 求出transform_1的逆变换。将右边界点从map转换到base_link下，便于根据车辆行进位姿进行边界点的提取
    pcl::transformPointCloud(front_Pts_added, front_Pts_added, transform_1.inverse());
    front_Pts_added.header.frame_id = "base_link";

    // 直通滤波，将right_Pts_added中处理过的点云滤除，否则该点云越来越大，浪费存储空间
    pass.setInputCloud(front_Pts_added.makeShared());
    pass.setFilterFieldName("x");
    pass.setFilterLimits(-10.0, 200.0); // 只保留前向这一段点云
    pass.filter(front_Pts_added);
    //再次转换到map坐标系下
    pcl::transformPointCloud(front_Pts_added, front_Pts_added, transform_1);
    front_Pts_added.header.frame_id = "map";*/
    ...
    pcl::toROSMsg(front_Pts_added, pc_publish_front);
    pc_publish_front.header.stamp = cloud_header.stamp;
    pc_publish_front.header.frame_id = "map";
    pub_front.publish(pc_publish_front);   // topic name: /road_front
